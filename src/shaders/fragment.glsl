...
layout(location = 2) in mat3 cam_view;
...
void main() {
    ...
    for (uint i = 0; i < uniforms.lights.size; i++) {
        brightness += dot(v_normal, normalize(cam_view * uniforms.lights.array[i].position)) * uniforms.lights.array[i].intensity * uniforms.lights.array[i].color;
    }
